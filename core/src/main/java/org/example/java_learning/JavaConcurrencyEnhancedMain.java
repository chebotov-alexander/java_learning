...
*/
    ...
    */
    ...
    /**
     * New approach to concurrency since Java 19.
     * Project Loom: Virtual Threads, Structured Concurrency and more.
     */
    public static class VirtualThreadsAndStructuredConcurrencyAkaProjectLoom {
        ...
        public static void main() {
            var bathTime = virtualThread("Bath time", bathing);
            var boilingTime = virtualThread("Boil some water", boilingWater);
            try { bathTime.join(); boilingTime.join(); } catch ( InterruptedException e ) {}
        }
        // Besides the factory method, there is new implementation of the java.util.concurrent.ExecutorService tailored on virtual threads, called java.util.concurrent.ThreadPerTaskExecutor. It creates a new virtual thread for every task submitted to the executor. Every call to the submit method requires a Runnable or a Callable<T> instance. The submit returns a Future<T> instance that we can use to join the underlying virtual thread.
        static void concurrentMorningRoutineUsingExecutors() {
            try (var executor = Executors.newVirtualThreadPerTaskExecutor()) {
                var bathTime = executor.submit(bathing);
                var boilingTime = executor.submit(boilingWater);
                bathTime.get();
                boilingTime.get();
            } catch ( ExecutionException | InterruptedException e) {}
            // output would be:
            //... - VirtualThread[#21]/runnable@ForkJoinPool-1-worker-1 | I'm going to take a bath
        }
        // Threads created this way do not have a name, and debugging errors without a name can be difficult. To overcome this problem use the newThreadPerTaskExecutor factory method that takes a ThreadFactory as a parameter.
        static void concurrentMorningRoutineUsingExecutorsWithName() {
            ...
            // output would be:
            //... - VirtualThread[#23,routine-1]/runnable@ForkJoinPool-1-worker-2 | I'm going to boil some water
        ...
        // To minimize pinned virtual threads blocking effect use Lock API (using java.util.concurrent.locks.ReentrantLock) instead of synchronized blocks. Such locks don’t pin the virtual thread, making the cooperative scheduling work again.
        static class SynchronizedVsLock {
            // Before.
            synchronized void synchronizedMethod() {
                println("Entering synchronized");
                sleep(Duration.ofSeconds(1L));
                println("Leaving synchronized");
            }
            // After.
            private final Lock lock = new ReentrantLock();
            void lockMethod() throws InterruptedException {
                if (lock.tryLock(10, TimeUnit.SECONDS)) {
                    try {
                        println("Entering lock");
                        sleep(Duration.ofSeconds(1L));
                        println("Leaving lock");
                    } finally { lock.unlock(); }
                }
            }
        }
        ...
        Alternatives.
         It is possible to emulate many of the features of scoped values with thread-local variables, albeit at some cost in memory footprint, security, and performance. Carrying the additional baggage of thread-local variables results in an implementation that is unduly burdensome, or an API that returns UnsupportedOperationException for much of its core functionality, or both. It is better, therefore, not to modify ThreadLocal but to introduce scoped values as an entirely separate concept.
        Motivation.
          Large Java programs typically consist of distinct and complementary components that need to share data between themselves. Some of them due to security reasons must noe share data by passing it as arguments from one function to another. In that case one should consider using scoped values.
          For example, a web framework might include a server component, implemented in the thread-per-request style, and a data access component, which handles persistence. Throughout the framework, user authentication and authorization relies on a Principal object shared between components. The server component creates a Principal for each thread that handles a request, and the data access component refers to a thread's Principal to control access to the database.
          Thread-local variables approach.
           Despite looking like an ordinary variable, a thread-local variable has multiple incarnations, one per thread; the particular incarnation that is used depends on which thread calls its get() or set(...) methods to read or write its value. Code in one thread automatically reads and writes its incarnation, while code in another thread automatically reads and writes its own distinct incarnation. Typically, a thread-local variable is declared as a final static field so it can easily be reached from many components. Here is an example of how the server component and the data access component, both running in the same request-handling thread, can use a thread-local variable to share a Principal.
                class Server {
           ...
           Problems with thread-local variables (design flaws that are impossible to avoid):
          ...
          Scoped values approach.
           ...
           The syntactic structure of the code using a scoped value:
             ...
             Together, where(...) and run(...) provide one-way sharing of data from the server component to the data access component. The scoped value passed to where(...) is bound to the corresponding object for the lifetime of the run(...) call, so PRINCIPAL.get() in any method called from run(...) will read that value. Accordingly, when Server.serve(...) calls user code, and user code calls DBAccess.open(), the value read from the scoped value (3) is the value written by Server.serve(...) earlier in the thread. The binding established by run(...) is usable only in code called from run(...). If PRINCIPAL.get() appeared in Server.serve(...) after the call to run(...), an exception would be thrown because PRINCIPAL is no longer bound in the thread.
        ...
        */
        ...
        static Runnable bathing = () -> {
            println("I'm going to take a bath");
            sleep(Duration.ofMillis(500L));
            println("I'm done with the bath");
        };
        static Runnable boilingWater = () -> {
            println("I'm going to boil some water");
            sleep(Duration.ofSeconds(1L));
            println("I'm done with the water");
        };
    }
}